...
# Note: Name the previous model in, wb_run_path
# Our hero should be custom and not cilrs
...
data_collect_dagger () {
  ...
  test_suites=lb_data \
  dataset_root=/media/shubham/data_nvme/Datasets/Daggers/futurestep_dagger1/ \
  actors.hero.coach=ppo \
  agent.ppo.wb_run_path=iccv21-roach/trained-models/1929isj0 \
  agent.ppo.wb_ckpt_step=null \
  actors.hero.driver=custom \
  n_episodes=80 inject_noise=false \
  dagger_thresholds.acc=0.2 \
  remove_final_steps=false \
  actors.hero.terminal.kwargs.max_time=300 \
...
#   wb_group=dagger5 \
#   test_suites=eu_data \
#   dataset_root=/media/shubham/data_nvme/Datasets/futurestep_dagger1/ \
